...
in
{
  ...
  config = mkIf cfg.enable {
    services.swayidle = let 
      ...
      # Programs
      swaylock = "${config.programs.swaylock.package}/bin/swaylock --image $(${config.services.awww.package}/bin/awww query | ${pkgs.coreutils}/bin/head --lines 1 | ${pkgs.coreutils}/bin/cut --delimiter ' ' --fields 9)";
